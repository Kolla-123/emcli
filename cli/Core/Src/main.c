...
static void uart_write(const char *text) {
  if (text != NULL) {
    HAL_UART_Transmit(&huart2, (uint8_t *)text, (uint16_t)strlen(text),
                      HAL_MAX_DELAY);
  }
}

static void uart_read_line(char *buffer, size_t buffer_len) {
  uint8_t byte = 0U;
  size_t index = 0U;
  ...
  while (1) {
    ...
    if (byte >= ' ' && byte <= '~' && index < (buffer_len - 1U)) {
...
void StartDefaultTask(void *argument)
{
  ...
  {
    uart_write("\r\nemcli> ");
    uart_read_line(command_buffer, sizeof(command_buffer));

    if (command_buffer[0] != '\0') {
      (void)cli_process_command(command_buffer, write_buffer,
                                sizeof(write_buffer));
